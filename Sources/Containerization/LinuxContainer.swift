                ...
                try await vm.withAgent { agent in
                    ...
                    if hasVirtiofsMount {
                        ...
                        if vm.virtiofsLayout == .perTag {
                            ...
                            // Additional mounts bind from /run/virtiofs/<tag>;
                            // the rootfs and the writable layer are mounted at
                            // the container's own paths directly, so their tags
                            // stay out.
                            let virtiofsAttachments = (vm.storage.containers[self.id]?.mounts ?? []).filter { $0.type == "virtiofs" }
